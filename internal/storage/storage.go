...
var ErrNameNotExists = errors.New("name does no exist")
var ErrNNamesNotExists = errors.New("names does no exist")
...
func (s *Storage) GetNameById(ctx context.Context, id int) (models.NameData, error) {
	...
	if err := rows.Scan(&name.Name, &name.Surname, &name.Patronymic, &name.Age, &name.Gender, &name.National); err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return models.NameData{}, ErrNameNotExists
		}
...
func (s *Storage) GetNameBySurname(ctx context.Context, surname string) ([]models.NameData, error) {
	rows, err := s.db.Query(ctx, `SELECT name, surname, patronymic, age, gender, "national" FROM names where surname = $1`, surname)
	if err != nil {
		return nil, err
	}
...
func (s *Storage) GetNamesByPatronymic(ctx context.Context, patronymic string) ([]models.NameData, error) {
	rows, err := s.db.Query(ctx, `SELECT name, surname, patronymic, age, gender, "national" FROM names where patronymic = $1`, patronymic)
...
func (s *Storage) GetNamesByAge(ctx context.Context, age int) ([]models.NameData, error) {
	rows, err := s.db.Query(ctx, `SELECT name, surname, patronymic, age, gender, "national" FROM names where age = $1`, age)
...
func (s *Storage) GetNamesByGender(ctx context.Context, gender string) ([]models.NameData, error) {
	rows, err := s.db.Query(ctx, `SELECT name, surname, patronymic, age, gender, "national" FROM names where gender = $1`, gender)
	if err != nil {
		return nil, err
	}
...
func (s *Storage) GetNamesByNational(ctx context.Context, national string) ([]models.NameData, error) {
	rows, err := s.db.Query(ctx, `SELECT name, surname, patronymic, age, gender, "national" FROM names where national = $1`, national)
	if err != nil {
		return nil, err
	}
